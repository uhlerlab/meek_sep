...
def clique_tree_policy(p: mean_match, sparse: int, central: bool=True) -> list:
    ...
    while not p.solved:
        ...
        if upstream_most:
            ...
            targets = set(random.sample(list(upstream_most), size))    
            ...
            while size>0 and top_trees:
                ...
                if sep:
                    if len(sep)<= size:
                        ...
                        targets = targets.union(random.sample(list(sep), size))
                        size = 0

            if size:
                add_targets = random.sample(list(partial_upstream_most-targets), size)
